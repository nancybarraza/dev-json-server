module.exports = {
  siteId: 1,
  externalId: "25251",
  name: "Site 1",
  plcId: 50,
  icianName: "ICIAN NAME",
  statusId: 4,
  connectionStatusId: 1,
  location: {
    locationId: 3,
    locationName: "Location Name",
    customer: {
      customerId: 8,
      companyName: "Company Name",
      primaryContact: {
        firstName: "Jonas Tester",
        lastName: "McQuinn Tester",
        email: "jmcquinn@bwcardiology.com",
        mobilePhone: "(858) 123-4567",
        officePhone: "(858) 987-6543",
      },
    },
    address: {
      addressId: 1,
      addressLine1: "Address Location 1 Tester",
      addressLine2: "Address Location 2 Tester",
      city: "City",
      state: "State",
      postalCode: 44540,
      latitude: 39.9697894,
      longitude: -75.1585807,
    },
  },
  address: {
    addressId: 1,
    addressLine1: "2345 El Rio Ave Tester",
    addressLine2: "",
    city: "Blue Bell Tester",
    state: "PA",
    postalCode: 18920,
    latitude: 39.9697894,
    longitude: -75.1585807,
  },
  tariffStructure: {
    tariff: "Pacific Gas Electric Tester",
    utility: "NEM2VMSH",
  },
  sharepointUrl: "https://www.microsoft.com/en-us/microsoft-365/sharepoint/collaboration?ms.officeurl=sharepoint&rtc=2",
};
